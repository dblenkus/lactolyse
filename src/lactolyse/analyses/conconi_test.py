...
class ConcoinAnalyses(BaseAnalysis):
    ...
    def _calculate_conconi_context(self, inputs, poly):
        # FTP value is normally somewhere around 70% of measured values,
        # so we limit the search interval to 60-80% to simplify the
        # prediction.
        lower_limit = inputs['power'][math.floor(len(inputs['power']) * 0.6)]
        upper_limit = inputs['power'][math.ceil(len(inputs['power']) * 0.8)]

        lower_pwr_range = list(filter(lambda pwr: pwr <= lower_limit, inputs['power']))
        upper_pwr_range = list(filter(lambda pwr: lower_limit < pwr, inputs['power']))

        lower_hr_range = inputs['heart_rate'][: len(lower_pwr_range)]
        upper_hr_range = inputs['heart_rate'][-len(upper_pwr_range) :]
        ...
        return {
            'power': ftp,
            'start_point': [inputs['power'][0], poly.poly(inputs['power'][0])],
            'end_point': [inputs['power'][-1], poly.poly(inputs['power'][-1])],
    ...
    def render_context(self, inputs):
        ...
        poly = FittedPolynomial(inputs['power'], inputs['heart_rate'])

        return {
            'inputs': inputs,
            'hr_poly': poly,
            'conconi': self._calculate_conconi_context(inputs, poly),
        }
